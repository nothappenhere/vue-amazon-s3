    ...
    <div class="bg-white shadow-lg rounded-lg p-6 mb-8 border">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <input
          ...
          required
          class="block w-full text-sm p-3 text-gray-500 border border-gray-300
        ...
        <button
          ...
          class="w-full bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700
            disabled:bg-gray-400 transition-all cursor-pointer"
        >
          {{ isUploading ? 'Uploading...' : 'Upload' }}
        </button>

        <p v-if="location" class="text-green-600 text-sm mt-2">
          File berhasil diupload:
          <a :href="location" target="_blank" class="underline">{{ location }}</a>
        </p>
        <p v-else class="text-red-600 text-center text-sm mt-2">
